---
title: "Autenticação Bearer na Ticto API v2 — tokens de acesso"
...
A Ticto API v2 utiliza **Bearer tokens** para autenticar todas as requisições. Cada chamada deve incluir um token válido no cabeçalho `Authorization` — sem ele, a API rejeitará a requisição imediatamente. Os tokens são gerados diretamente no painel da Ticto e carregam as permissões (escopos) que você definir no momento da criação.
...
```bash
curl https://api.ticto.com.br/api/v2/products \
